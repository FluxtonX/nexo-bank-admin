...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender: { name: "Nexo Security", email: "noreply@cdntbank.com" },
        to: [{ email: email }],
        ...
        htmlContent: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px;">
            ...
            <p style="color: #475569; font-size: 16px;">A login attempt was made to the Nexo Admin Panel. Please use the following 6-digit code to complete your sign in:</p>
